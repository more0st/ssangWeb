...
function moveToUrl() {
	...
	let setyear = f.yearSelector.value;
	let setmonth = f.monthSelector.value;
	
	document.location.href="calendar.jsp?year=<%=year%>&month=<%=month+1%>"
}
	 ...
	 -->
	 	<form name="searchForm" action="ex06_ok.jsp" method="post">
			<select name="yearSelector">
				...
				for(int i=0; i<10; i++){ 
					if(y==year){
						out.print("<option value='"+y+"' selected>"+y+"년</option>");
					} else {
						out.print("<option value='"+y+"'>"+y+"년</option>");
					}
					y++;
				}
				%>
			...
			<select name="monthSelector" )>
				...
				for(int i=0; i<12; i++){ 
					if(m==month){
						out.print("<option value='"+m+"' selected>"+m+"월</option>");
					} else {
						out.print("<option value='"+m+"'>"+m+"월</option>");
					}
					m++;
				}
				%>
